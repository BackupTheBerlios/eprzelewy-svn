...
//               root.setAttribute("title", lineEdit17->text());
 ...
 case 4:
  ...
  file.close ();
    ...
    if (kwt == "") kwt = "-";
    ret = fn2 + "|" + QDate::currentDate ().toString ("dd-MM-yyyy") + "|US" +
	comboBox1->currentText() + "|" + cbNazwa->currentText() + "|" + kwt;
...
void pUS::idChanged()
{
 ...
 case 0:
   ...
   break;
 case 1:
   nipEdit->setText( settings.readEntry( "przelewy/user/regon" ) );
   break;
 case 2:
   nipEdit->setText( settings.readEntry( "przelewy/user/regon" ) );
   break;
